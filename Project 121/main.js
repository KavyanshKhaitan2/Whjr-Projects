...
for (let i = 0; i < cardnamelist.length; i++) {
  ...
  template = template.replaceAll("{card-index}", i)
  if(c.cards[i].imagePath) {
    ...
    template = template.replaceAll("{imageHeight}", c.cards[i].imageHeight)
  } else {
    template = template.replaceAll("{card-image-alt}", "")
    template = template.replaceAll("{card-image}", "")
    template = template.replaceAll("{imageHeight}", "")
  }
  document.getElementById("cards").innerHTML += template;
  print(body.querySelector("#card-"+i).querySelector(".card-image").style.height = c.cards[i].imageHeight)
}
